...
class TrackManagerNode(Node):
    def __init__(self) -> None:
        ...
        self.centroid_sub = self.create_subscription(PoseArray, '/tracking/cluster_centroids', self.centroid_callback, 10)
        self.track_marker_pub = self.create_publisher(MarkerArray, '/tracking/track_markers', 10)
        self.clear_tracks_srv = self.create_service(Trigger, '/tracking/clear_tracks', self.clear_tracks)
    ...
    def update_tracks(self, detections: List[np.ndarray], stamp_sec: float) -> None:
        ...
        for distance, track_id, detection_index in candidate_matches:
            del distance
    ...
    def publish_track_markers(self, stamp) -> None:
        ...
        for track in sorted(self.tracks.values(), key=lambda item: item.track_id):
            ...
            text.text = f'T{track.track_id} age:{track.age} v:{speed:.2f}m/s'
