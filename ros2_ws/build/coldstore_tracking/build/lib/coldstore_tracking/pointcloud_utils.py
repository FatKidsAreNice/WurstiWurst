...
def points_to_voxel_keys(points_xyz: np.ndarray) -> Set[VoxelKey]:
    if points_xyz.size == 0:
        return set()
    ...
    voxel_indices = np.floor(points_xyz).astype(np.int32)
...
def bounds_from_points(points_xyz: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    return points_xyz.min(axis=0), points_xyz.max(axis=0)